...
<div class="container py-4">
    @if(!isset($asset) || !$asset)
        <div class="alert alert-danger text-center">
            <strong>{{ $errorMessage ?? 'Asset tidak ditemukan atau sudah dihapus.' }}</strong>
        </div>
        <a href="{{ route('Asset_Master.index') }}" class="btn btn-outline-primary mt-3">
            <i class="bi bi-arrow-left"></i> Kembali ke Asset Master
        </a>
    @else
                ...
                <div>
                    ...
                    <div class="fs-5 fw-semibold text-white-50">
                        ...
                        @if(method_exists($asset, 'trashed') && $asset->trashed())
                            <span class="badge bg-danger ms-2">Asset sudah dihapus</span>
                        @endif
                    </div>
                </div>
            ...
            <div class="asset-summary">
                <div class="row g-4 asset-metadata">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-2"><strong>Date Delete:</strong> 
                            <span class="value">
                                @if(method_exists($asset, 'trashed') && $asset->trashed())
                                    {{ $asset->deleted_at ? $asset->deleted_at->format('d-m-Y H:i') : '-' }}
                                @else
                                    {{ $dateDelete ?? '-' }}
                                @endif
                            </span>
